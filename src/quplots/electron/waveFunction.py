import numpy as np
from .scipy_compat import sph_harm_compat
...
def normalized_wf(d, n, l, m):
    ...
    psi = radial(r, n, l) * sph_harm_compat(m, l, 0, np.arctan(x / (y + 1e-7)))
    psi_sq = np.abs(psi) ** 2
...
def normalized_wf3D(n, l, m):
    ...
    psi = radial(r, n, l) * np.real(sph_harm_compat(m, l, phi, theta))
    return psi
...
def normalized_wf3D_complex(n, l, m):
    ...
    psi = (
        ...
        * np.real(sph_harm_compat(m, l, phi, theta))
    )
    return psi
